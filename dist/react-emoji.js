(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["ReactEmoji"] = factory(require("react"));
	else
		root["ReactEmoji"] = factory(root["React"]);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var annotations = _interopRequire(__webpack_require__(2));

	var emoticons = _interopRequire(__webpack_require__(3));

	var escapeStringRegexp = _interopRequire(__webpack_require__(4));

	var assign = _interopRequire(__webpack_require__(5));

	var compact = _interopRequire(__webpack_require__(6));

	var ReactEmoji = function () {
	  ...
	  var getEscapedKeys = function (hash) {
	    return Object.keys(hash).map(function (x) {
	      return escapeStringRegexp(x);
	    }).join("|");
	  };
	  ...
	  var buildImageUrl = function (hex, options) {
	    if (options.host) {
	  ...
	  var emojifyTextToSingleEmoji = function (text, options) {
	    ...
	    return React.createElement("img", assign(options.attributes, {
	      src: buildImageUrl(hex, options)
	    }));
	  };

	  var emojifyText = function (text, options) {
	    var _ref = options.useEmoticon ? emojiWithEmoticons : emojiWithoutEmoticons;

	    var delimiter = _ref.delimiter;
	    var dict = _ref.dict;

	    return compact(text.split(delimiter).map(function (word, index) {
